...
这里有一个重要前提：1−φ 不能等于 0。如果 φ=1，均值公式失效，序列也不再围绕某个固定均值波动。这正是单位根问题的最简单来源。
...
若 t 期发生一个单位冲击，即 εₜ 增加 1，而其他未来冲击保持不变，则当期 yₜ 增加 1。到了下一期，yₜ 的增加通过系数 φ 传递到 yₜ₊₁，因此下一期影响为 φ。再往后一期，影响继续乘以 φ，变成 φ²。
...
这个方差只有在 |φ|<1 时才是有限的。如果 |φ|≥1，分母不再给出正的有限方差，序列不能成为协方差平稳过程。
...
这就解释了为什么平稳 AR(1) 的 ACF 通常表现为拖尾：只要 |φ|<1，φᵏ 会逐渐衰减，但不会在某个有限阶之后突然等于零。
...
这正是一个无限阶 MA 表示。只要 |φ|<1，系数 φʲ 会逐渐衰减，因此无限和是稳定的。这个例子说明：AR 模型和 MA 模型并不是彼此割裂的两类模型，而是描述同一动态过程的两种方式。
