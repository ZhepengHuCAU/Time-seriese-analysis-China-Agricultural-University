...
设 {yₜ} 是一个弱平稳序列，其均值为 μ。k 阶自协方差定义为：

    γₖ = Cov(yₜ, yₜ₋ₖ)
       = E[(yₜ − μ)(yₜ₋ₖ − μ)]

由于弱平稳性要求协方差只依赖两个观测之间的距离，而不依赖具体发生在第几期，因此 γₖ 只随滞后阶数 k 改变。换言之，γₖ 描述的是相隔 k 期的两个观测值之间的线性联系。
...
自协方差的大小受变量单位影响，不便直接比较。例如，若把价格从“元”改为“万元”，自协方差的数值会发生比例变化。为了得到不受计量单位影响的指标，需要将自协方差标准化。k 阶自相关系数定义为：
...
因此需要用样本估计总体自相关。设样本均值为：

    ȳ = (1/T)∑yₜ

其中求和范围为 t = 1, 2, ..., T。k 阶样本自协方差通常写为：

    γ̂ₖ = (1/T)∑(yₜ − ȳ)(yₜ₋ₖ − ȳ)

其中求和范围为 t = k+1, k+2, ..., T。相应地，k 阶样本自相关系数为：
...
**本节小结。** 自协方差函数描述序列在不同滞后距离上的线性联系；自相关函数将这种联系标准化，使不同变量和不同样本更便于比较；样本 ACF 是实际诊断白噪声和识别模型阶数的基本工具；PACF 则刻画控制中间滞后后的直接相关。二者共同构成识别 AR、MA 和 ARMA 模型的基本图形工具。
